...
db.execute('create table links ' +
           '(id integer, submitter_id integer, ' +
            'submitted_time integer, votes integer,' +
            'title text, url text)')
for ltuple in links:
    db.execute('insert into links values (?,?,?,?,?,?)', ltuple)
...
def getKey(linkItem):
    return linkItem.submitted_time;
  
def query():
  sortedList=sorted(links, key=getKey)
...
# three versions to generate list
def get_id_list():
...
dict = build_link_index()

def link_by_idv2(link_id):
...
newLink = Link(50, 60398, 1334014208.0, 109,
         "C overtakes Java as the No. 1 programming language in the TIOBE index.",
         "http://pixelstech.net/article/index.php?id=1333969280")

add_new_link(newLink)
print dict.get(50);
